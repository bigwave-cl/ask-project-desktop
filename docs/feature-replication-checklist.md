...
- [ ] 复刻 `tab.vue` 的显示条件：分组数量大于 1 时显示。
- [ ] 复刻 `buildGroupList`：固定“全部”，后接配置分组。
- [ ] 复刻 active group 不存在时自动回到 `all`。
- [ ] 复刻当前分组无项目与完全无分组两种空状态。
- [ ] 复刻空状态按钮：添加分组、导入配置、导入文件夹、导入工作区。
- [ ] 复刻重复路径去重逻辑。
- [ ] 复刻当前窗口路径匹配 `isSamePath`。
