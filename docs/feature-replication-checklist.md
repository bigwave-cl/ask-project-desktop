...
- [ ] 收敛现有原生 `<button>` / `<input>`：逐步替换为 `ui/Button`、`ui/Input` 或基于它们封装。
  ...
  - 进度：基础 UI、HUD 与偏好设置的边框、多层背景、线条和阴影已收敛为 `border-apm-*`、`bg-apm-*`、`shadow-apm-*` Tailwind 工具类，并按背景、阴影、边框、变体桥接、动画类型拆分到 `apps/desktop/src/app/themes` 后由 `globals.css` 统一引入。
