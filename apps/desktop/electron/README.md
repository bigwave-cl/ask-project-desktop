...
日常开发从仓库根目录启动，脚本会同时处理 React/Next 页面和 Electron 桌面壳：
...
Electron 内部排障时，可以在 4000 已经运行后直接启动 desktop 包内脚本：
...
```bash
ASK_PROJECT_ELECTRON_URL=http://localhost:4000 pnpm --filter desktop electron:dev
```
